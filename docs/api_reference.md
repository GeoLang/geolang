...
The MCP token carries the private claim `geolang_use: "mcp"`, which only opens this endpoint, and the source platform role in `source_role`. Neither reaches a tool or downstream service. Before each execution geolang mints a role-free JWT carrying the same `sub`, `token_use: "tool"`, and a JSON string array named `scope`. Its `exp` is the earlier of the MCP token's expiry or five minutes from exchange. Four tools have scopes mapped: `ptolemy_query` gets `ptolemy:read`, `list_tilesets` gets `tiletopia:read`, `asset_readings` gets `agora:read`, and `run_workflow` gets `geodukt:run`. Every other tool gets an empty array. Exact scope strings are enforced downstream, with no role fallback. The source role also caps the exchange: a viewer can receive read scopes, but not `geodukt:run`.
...
41 tools live under [`src/agents/tools/`](../src/agents/tools/). They are loaded from disk on every manifest or execution request. Categories:
...
**Routing & accessibility** — `compute_route` (itinera-first, Valhalla fallback), `calculate_isochrones`, `service_gap`, `score_sites`, `trade_area` (one travel-time catchment per candidate site, with the population, competitors, anchors and census figures inside it).
