...
An output file has a lifetime, since the caller's own `DELETE /outputs/{filename}` reaches only their directory and a file no tool announced is deleted by nothing. [`src/api/outputs_retention.py`](../src/api/outputs_retention.py) walks every caller directory at API startup and once a day after that, deletes the regular files at any depth last written more than `GEOLANG_OUTPUTS_RETENTION_DAYS` ago, and removes a directory it emptied. With `GEOLANG_USER_DATA_RETENTION_DAYS` set it does the same to the caller directories under `user_data/`. It leaves each caller's upload catalogue alone, and `load_catalogue` drops an entry whose file is gone, so a sweep never has to rewrite a catalogue an upload may be writing at the same time. It runs in the API server and not in the executor, which mounts the same volume, so one process is the only deleter. Files in the outputs parent itself, the ones that predate the per-caller split, are left where they are: nothing serves them either.
