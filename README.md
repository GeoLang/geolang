...
Place names are looked up in geokode at `GEOKODE_URL`, the platform geocoder, and never in Nominatim. The outline of a named place or feature is fetched from Overpass by the OSM type and id geokode returns. With `GEOKODE_URL` unset, `geocode_place` answers from Natural Earth populated places and every other tool that takes a place name refuses it.

Requests to Overpass are paced across every tool run in the process that runs tools and its workers: at most one request per host every 1.1 seconds. The last request time sits in a locked file per host under the temp directory, because every tool run is its own process. That covers the tools' own requests and the ones osmnx sends.
